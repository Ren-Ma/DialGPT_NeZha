...
setproctitle.setproctitle('gpt2_mmi_nz')
maxlen = 512
batch_size = 48
...
# nezha配置
config_path = os.path.join(file_root,'nezha_gpt/nezha_gpt_dialog/config.json')
checkpoint_path = os.path.join(file_root,'nezha_gpt/nezha_gpt_dialog/model.ckpt')
dict_path = os.path.join(file_root,'nezha_gpt/nezha_gpt_dialog/vocab.txt')


def corpus(data_dir, reverse=False):
    """循环读取语料
    """
    while True:
        data_all = []
...
# 补充词表
compound_tokens = []
for l in open(os.path.join(file_root,'user_tokens.csv'), encoding='utf-8'):
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        while True:
            try:
                model.save_weights(os.path.join(file_root,'latest_model_mmi.weights'))
                break
            except:
                print(u'保存失败，正在重试...')
...
if __name__ == '__main__':
    ...
    evaluator = Evaluator()
    train_generator = data_generator(corpus(data_dir,reverse=True), batch_size)
    # 加载最后模型权重
    model.load_weights(os.path.join(file_root, 'latest_model_mmi.weights'))
    model.fit_generator(
