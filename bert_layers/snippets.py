...
class AutoRegressiveDecoder(object):
    ...
    def wraps(default_rtype='probas', use_states=False):
        ...
        def actual_decorator(predict):
            def new_predict(
                ...
                prediction = predict(self, inputs, output_ids, states) # -> nezha_gpt_train/ChatBot/predict @ ren ma 2022.11.11
    ...
    def predict_batch(self, inputs, output_ids, states=None, rtype='logits'):
        """用户需自定义递归预测函数
        说明：rtype为字符串logits或probas，用户定义的时候，应当根据rtype来
              返回不同的结果，rtype=probas时返回归一化的概率，rtype=logits时
              则返回softmax前的结果或者概率对数。
        返回：二元组 (得分或概率, states)*batch_size
        """
        raise NotImplementedError
    ...
    def random_sample(self, inputs, n, topk=None, topp=None, states=None, min_ends=1):
        ...
        results = []
        for step in range(self.maxlen):
            probas, states = self.predict(  # -> AutoRegressiveDecoder/wraps/actual_decorator/ @ ren ma 2022.11.11
    ...
    def random_sample_batch(self, inputs, n, topk=None, topp=None, states=None, min_ends=1, tokenizer=None):
        """随机采样n个结果
        说明：非None的topk表示每一步只从概率最高的topk个中采样；而非None的topp
             表示每一步只从概率最高的且概率之和刚好达到topp的若干个token中采样。inputs为(token_ids,segment_ids)*batch_size
        返回：n个解码序列组成的list。
        """
        inputs = [np.array(i) for i in inputs]  #
        output_ids = np.empty((len(inputs[0]),0), dtype=int)
        results = []
        flag_map = []
        for step in range(self.maxlen):
            probas, states = self.predict_batch(inputs, output_ids, states, 'probas')  # 计算当前概率
            probas /= probas.sum(axis=1, keepdims=True)  # 确保归一化
            if step == 0:  # 第1步预测后将结果重复n次
                """
                ...
                # 对于[UNK]的概率设为无穷小，也就是说模型的预测结果不可能是[UNK]这个token，增加惩罚性，开头不能出现标点，不能出现【的你】，将其中对应的概率转为-float('Inf')
                for item in [',','，','.','。','的']:
                    probas[:,tokenizer._token_dict.get(item,'[UNK]')] = 0
                probas = np.repeat(probas, n, axis=0)
                inputs = [np.repeat(i, n, axis=0) for i in inputs]
                output_ids = np.repeat(output_ids, n, axis=0)
            if topk is not None:
            ...
            if output_ids.shape[1] >= self.minlen:  # 最短长度判断
                flag = (end_counts == min_ends)  # 标记已完成序列
                if flag.any():  # 如果有已完成的
                    flag_map.append(flag)
                    for ids in output_ids[flag]:  # 存好已完成序列
                        results.append(ids)  # 如果是batch输入的话，需要维护一个对应关系。
                    flag = (flag == False)  # 标记未完成序列
                    inputs = [i[flag] for i in inputs]  # 只保留未完成部分输入
                    output_ids = output_ids[flag]  # 只保留未完成部分候选集
                    end_counts = end_counts[flag]  # 只保留未完成部分end计数
        ...
        for arr in flag_map:
            arr_re = list(arr)
            for i in map_sort:
                arr_re.insert(i, True)
            for idx, a in enumerate(arr_re):
                if idx not in map_final and a:
                    map_final.append(idx)
                    map_sort = sorted(map_final, key=lambda x:x, reverse=False)  # 从小到大排序，可以确保插入的正确性
        # 如果还有未完成序列，直接放入结果,不产生与输入一致的结果
        # for ids in output_ids:
        #     results.append(ids)
        # 对results根据flag_map排序
        # print(len(map_final),len(results))
        res_final = [None]*(max(map_final)+1)
        for idx,res in enumerate(results):
            res_final[map_final[idx]] = res
        # 返回结果
        return res_final
