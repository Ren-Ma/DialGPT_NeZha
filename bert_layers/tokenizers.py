...
from src.TextGen.bert_layers.snippets import is_string, is_py2
from src.TextGen.bert_layers.snippets import open
...
class BasicTokenizer(object):
    ...
    def tokenize(self, text, maxlen=None):
        ...
        if self._token_start is not None:
            tokens.insert(0, self._token_start)  # 在tokens前加上[CLS]
        if self._token_end is not None:
            tokens.append(self._token_end)  # 在tokens后加上[SEP]
